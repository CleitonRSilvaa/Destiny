...
const FormManager = {
    ...
    validateForm(fields, alertId, isCreating) {
        ...
        const editSenhaValue = document.getElementById('editSenha') ? document.getElementById('editSenha').value.trim() : '';
        const editSenhaConfimeValue = document.getElementById('editSenhaConfime') ? document.getElementById('editSenhaConfime').value.trim() : '';
        const senhaValue = document.getElementById('senha') ? document.getElementById('senha').value.trim() : '';
        const senhaConfimeValue = document.getElementById('senhaConfime') ? document.getElementById('senhaConfime').value.trim() : '';

        // Se apenas um dos campos de senha de edição estiver preenchido
        if ((editSenhaValue && !editSenhaConfimeValue) || (!editSenhaValue && editSenhaConfimeValue)) {
            this.showAlert(alert, "Ambos os campos de senha de edição devem ser preenchidos ou ambos devem ser deixados em branco!");
            return false;
        }
        ...
        for (let field of fields) {
            ...
            if ((field === "email" || field === "editEmail") && !this.isValidEmail(value)) {
...
document.getElementById("staticBackdrop").addEventListener("hidden.bs.modal", function () {
    FormManager.clearForm(["nome", "email", "senha", "cpf", "senhaConfime", "tipoConta"], "usuarioAlert", { tipoConta: "ESTOQUISTA" });
});

document.getElementById("modalEdicao").addEventListener("hidden.bs.modal", function () {
    FormManager.clearForm(["editId", "editNome", "editEmail", "editCpf", "editSenha", "editSenhaConfime", "editStatus", "editTipoConta"], "edicaoUsuarioAlert");
});
...
function makeRequest(type, url, data, successMessage) {
 console.log(data);
    $.ajax({
        type: type,
        url: `${BASE_URL}${url}`,  // Usando a BASE_URL como sugerido anteriormente
        data: JSON.stringify(data),
        contentType: "application/json; charset=utf-8",

        success: function (response) {
            if ([200, 201].includes(response.status) && response.message === "sucess") {
                Swal.fire({
                    position: "top-end",
                    icon: "success",
...
function updateStatusConta(button) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            makeRequest("POST", "/usuario/updateStatus", data, "Status atualizado com sucesso!");
        }
    });
}
...
function submitForm() {
    if (!FormManager.validateForm(["nome", "email", "senha","senhaConfime", "cpf", "tipoConta"], "usuarioAlert", true)) {
        return;
    }
...
function updateUsuario() {
    if (!FormManager.validateForm(["editId", "editNome", "editEmail", "editCpf", "editSenha", "editTipoConta", "editStatus"], "edicaoUsuarioAlert", false)) {
        return;
    }
    const mapping = {
        editId: "id",
    ...
    const data = getFormData(["editId", "editNome", "editEmail", "editCpf", "editSenha", "editTipoConta", "editStatus"], mapping);
    makeRequest("PUT", "/usuario/update", data, "Usuário atualizado com sucesso!");
}
